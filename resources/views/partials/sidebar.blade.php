<nav class="flex-1 mt-4 px-3 space-y-2">
    ...
    <a hx-get="/" hx-target="#main-content" hx-push-url="true"
        ...
        <svg xmlns="http://www.w3.org" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" 
            class="w-6 h-6 mr-3 transition-colors {{ request()->is('/') ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400' }}">
    ...
    <!-- Notifikasi -->
    <a hx-get="/notification-detail" hx-target="#main-content" hx-push-url="true"
        class="flex items-center p-3 rounded-lg cursor-pointer transition group 
        {{ request()->is('notification-detail*') 
            ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' 
            : 'text-slate-600 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800 hover:text-blue-600 dark:hover:text-blue-400' 
        }}">
        
        <svg xmlns="http://www.w3.org" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" 
            class="w-6 h-6 mr-3 transition-colors {{ request()->is('notification-detail*') ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 group-hover:text-blue-600 dark:group-hover:text-blue-400' }}">
