@forelse($todos as $todo)
    <div class="p-4 bg-white dark:bg-slate-900 rounded-xl border border-gray-100 dark:border-slate-800 group shadow-sm flex items-center justify-between transition-all duration-300 hover:border-blue-200 dark:hover:border-blue-900/50 {{ $todo->is_completed ? 'grayscale-[0.4] opacity-80' : '' }}">
                ...
                <div class="flex items-center gap-2 mt-1">
                    ...
                    @if($todo->due_date)
                        <div class="flex items-center gap-2">
                            ...
                            <div class="flex items-center gap-1">
                                @php
                                    $isOverdue = $todo->due_date < now()->toDateString() && !$todo->is_completed;
                                @endphp
                                <svg class="w-3.5 h-3.5 {{ $isOverdue ? 'text-red-500' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300">
            <button 
                ...
                @click.prevent="
                    if({{ $todo->is_completed ? 'true' : 'false' }}) {
                        // Jika membatalkan selesai (Warning)
                        $dispatch('open-confirm-warning', {
                            title: 'Aktifkan Kembali?', 
                            message: 'Tugas akan dikembalikan ke daftar aktif.', 
                            confirmText: 'Ya, Aktifkan',
                            target: $el 
                        })
                    } else {
                        // Jika menyelesaikan tugas (Success)
                        $dispatch('open-confirm-success', {
                            title: 'Selesaikan Tugas?', 
                            message: 'Selamat! Anda telah menyelesaikan tugas ini.', 
            ...
            <button 
                ...
                hx-include="#csrf-holder [name=_token]" 
                hx-trigger="confirmed" 
                @click="$dispatch('open-confirm-danger', { 
                    title: 'Hapus Tugas?', 
    ...
    @if ($loop->last && $todos->hasMorePages())
        <div id="replace-me-with-next-page" class="py-4 flex justify-center">
            <button 
                hx-get="{{ $todos->nextPageUrl() }}" 
                hx-include="#csrf-holder [name=_token]" 
                hx-trigger="click" 
                hx-target="#replace-me-with-next-page" 
                hx-swap="outerHTML"
                hx-indicator="#load-more-spinner"
                class="px-6 py-2 bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold rounded-full shadow-lg transition-all active:scale-95">
                DEBUG: Load Data Selanjutnya (Page {{ $todos->currentPage() + 1 }})
            </button>
        </div>
    @endif
@empty
